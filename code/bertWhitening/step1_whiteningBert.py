...
def build_model():
    tokenizer = BertTokenizer.from_pretrained(cf.tokenzier_model)
    model = BertForMaskedLM.from_pretrained(cf.bert_train_output)
...
def sents_to_vecs(sents, tokenizer, model):
    ...
    with torch.no_grad():
        ...
        fault_data_loader = DataLoader(dataset=sents_data, batch_size=150, shuffle=True)
...
def main():
    ...
    import pickle
    f = open(os.path.join(cf.bert_whitening_train_output,'kernel.pkl'), 'wb')
    pickle.dump(kernel, f)
    f.close()
    f = open(os.path.join(cf.bert_whitening_train_output,"bias.pkl"), 'wb')
    pickle.dump(bias, f)
    f.close()
