...
from code.experiment.config import Bug as cf
class BugDetector:
    def __init__(self, output_dir, pooler_type):
        ...
        self.threshold = 0.95
        self.tokenizer = BertTokenizer.from_pretrained(bt.tokenzier_model)
        self.model = BertForMaskedLM.from_pretrained(bt.bert_train_output)
...
if __name__ == '__main__':
    # todo
    test_data = MyDataset(data_path=os.path.join(cf.Bug.bug_construct_data_path,"test_data_my.pkl"))
    test_data_loader = DataLoader(dataset=test_data, batch_size=64, shuffle=True)
    # todo
    fault_data = MyDataset(data_path=os.path.join(cf.Bug.bug_construct_data_path,"fault_data_my.pkl"))
    ...
    if len(args) == 1:
        ...
        pooler_type = args[1]
    detector = BugDetector(cf.Bug.bug_construct_data_path, pooler_type=pooler_type)
