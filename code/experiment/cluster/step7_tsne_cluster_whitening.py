...
if __name__ == '__main__':
    ...
    if len(args) == 1:
        ...
        model.to(device)
        with open(os.path.join(cf.bert_whitening,"kernal.pkl")) as f:
            kernel = pickle.load(f)
        with open(os.path.join(cf.bert_whitening,"bias.pkl")) as f:
            bias = pickle.load(f)
    else:
        pooler_type = args[1]
    if len(args) == 2:
        if args[2] == "mirror_bert":
            ...
            model.to(device)
            with open(os.path.join(cf.bert_mirror_whitening_train_output, "kernal.pkl")) as f:
                kernel = pickle.load(f)
            with open(os.path.join(cf.bert_mirror_whitening_train_output, "bias.pkl")) as f:
                bias = pickle.load(f)
        else:
            print("first_args,you can choose [avg_first_last,avg_top, avg], second arg you choose one in :[mirror_bert],solibert model is default")
            sys.exit()
